...
export default function AgentCard({
  ...
  return (
    <article className={`rounded-xl border bg-panel p-4 ${accentStyles[accent]}`}>
      <div className="flex items-start justify-between gap-3">
        <div>
          <h4 className="text-sm font-semibold text-textMain">{name}</h4>
          <p className="mt-1 text-[11px] uppercase tracking-[0.12em] text-textSub">{role}</p>
        </div>
        <div className="rounded-md border border-border bg-panel2 px-2 py-1 text-[11px] font-medium text-textSub">Signal {displaySignal}</div>
      </div>

      <p className="mt-3 text-xs leading-5 text-textMain">{activity}</p>

      <div className="mt-3 grid grid-cols-2 gap-2">
        {primaryMetrics.map((metric) => (
          <div key={metric.label} className="rounded-md border border-border bg-panel2 px-2.5 py-2">
            <p className="text-[10px] uppercase tracking-[0.1em] text-textSub">{metric.label}</p>
            <p className="mt-1 text-xs font-semibold text-textMain">{metric.value}</p>
          </div>
        ))}
      </div>
